use saa_common::{
    ensure, AuthError, Binary, CredentialError, CredentialInfo, CredentialName, Identifiable,
    String, Verifiable,
};
...
impl Verifiable for PasskeyCredential {
    ...
    fn validate(&self) -> Result<(), AuthError> {
            ...
            self.signature.len() > 0
                ...
                && self.message().len() > 0,
            CredentialError::MissingData(PasskeyName)
        );
        ensure!(
            self.authenticator_data.len() >= 37,
            CredentialError::InvalidProperty(
                PasskeyName,
                "authenticator_data".to_string(),
                "must be at least 37 bytes long".to_string()
            )
        );
        ensure!(
            self.client_data.ty == "webauthn.get",
            CredentialError::InvalidProperty(
                PasskeyName,
                "client_data.type".to_string(),
                "must be 'webauthn.get'".to_string()
            )
        );
        Ok(())
    }
    ...
    fn verify(
        &self,
        #[cfg(feature = "cosmwasm")] deps: saa_common::wasm::Deps,
    ) -> Result<CredentialInfo, AuthError> {
        let res = true;
        #[cfg(all(
            any(feature = "native", feature = "no_api_r1"),
